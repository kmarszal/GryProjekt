...
public class EnemyPathfinder : MonoBehaviour {
    ...
    {
        GridReference = GameManager.GetComponent<Grid>();//Get a reference to the game manager
		EnemyController = GetComponent<EnemyController>();
    }

    private void Update()//Every frame
    {
        FindPath(transform.position, TargetPosition.position);//Find a path to the goal
    }

    void FindPath(Vector3 a_StartPos, Vector3 a_TargetPos)
    {
		if(Vector3.Distance(a_StartPos, a_TargetPos) <= ViewRadius)
		{
    ...
    void GetFinalPath(Node a_StartingNode, Node a_EndNode)
    {
        ...
        EnemyController.FinalPath = FinalPath;//Set the final path
