...
public class Grid : MonoBehaviour
{
			...
			for (int y = -1; y <= 1; ++y)
			{
				if(x != 0 || y != 0)
				{
					...
					if (icheckX >= 0 && icheckX < iGridSizeX && 
						icheckY >= 0 && icheckY < iGridSizeY && 
						allowDiagonalMovement(x, y, icheckX, icheckY))//If the XPosition and YPosition is in range of the array
					{
	...
	private bool allowDiagonalMovement(int x, int y, int icheckX, int icheckY)
	{
		if (x == 1) 
		{
			if(!NodeArray[icheckX, icheckY - y].bIsWall) {
				return false;
			}
		}
		else if (x == -1)
		...
		{
			if(!NodeArray[icheckX - x, icheckY].bIsWall) {
				return false;
			}
		}
		else if (y == -1) 
